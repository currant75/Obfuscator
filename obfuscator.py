...
Input/Output: Local Storage Only
"""
...
def process_file(input_path, output_path, opts):
    ...
    print(f"  🔒 Strength: {obfuscator.stats['strength']}% | Layers: {obfuscator.stats['layers']}/24\n")
    return True
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
╔══════════════════════════════════════════════════════════════╗
║  CARA PENGGUNAAN DARI PENYIMPANAN LOKAL                      ║
╠══════════════════════════════════════════════════════════════╣
║  1 FILE:                                                     ║
║    python3 lua_obfuscator_max.py -i /sdcard/script.lua       ║
║           -o /sdcard/protected.lua --max                     ║
║                                                              ║
║  BATCH (semua .lua di folder):                               ║
║    python3 lua_obfuscator_max.py -d /sdcard/lua_files/       ║
║           -o /sdcard/output/ --max                           ║
║                                                              ║
║  TINGKAT TERAMAN (MAXIMUM):                                  ║
║    python3 lua_obfuscator_max.py -i in.lua -o out.lua --max  ║
║        --watermark "MyScript v1.0" -v                        ║
╚══════════════════════════════════════════════════════════════╝
        """
    )
    ...
    group.add_argument('-i', '--input', help='File Lua input (single file)')
    group.add_argument('-d', '--directory', help='Folder berisi file .lua (batch mode)')

    parser.add_argument('-o', '--output', required=True, help='File output (single) atau folder output (batch)')
    parser.add_argument('--max', action='store_true', help='Maximum security mode (semua layer extreme)')
    parser.add_argument('--watermark', type=str, default='', help='Tambah watermark')
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help='Tampilkan detail tiap layer')
    parser.add_argument('--suffix', type=str, default='_protected', help='Suffix untuk file output batch')

    # Layer toggles
    parser.add_argument('--no-string-enc', action='store_true', help='Matikan string encryption')
    parser.add_argument('--no-var-rename', action='store_true', help='Matikan variable renaming')
    parser.add_argument('--no-control-flow', action='store_true', help='Matikan control flow flattening')
    parser.add_argument('--no-vm', action='store_true', help='Matikan VM obfuscation')
    parser.add_argument('--no-bytecode', action='store_true', help='Matikan bytecode encryption')
    parser.add_argument('--no-dynamic-loader', action='store_true', help='Matikan dynamic loader')
    parser.add_argument('--no-anti-tamper', action='store_true', help='Matikan anti-tamper')
    parser.add_argument('--no-anti-debug', action='store_true', help='Matikan anti-debug')
    parser.add_argument('--no-anti-memory', action='store_true', help='Matikan anti-memory scan')
    parser.add_argument('--no-const-pool', action='store_true', help='Matikan encrypted constant pool')
    parser.add_argument('--no-anti-dump', action='store_true', help='Matikan anti-dump')
    parser.add_argument('--no-mutation', action='store_true', help='Matikan code mutation')
    parser.add_argument('--no-code-split', action='store_true', help='Matikan code splitting')
    parser.add_argument('--no-compress', action='store_true', help='Matikan compression')
    parser.add_argument('--no-env-finger', action='store_true', help='Matikan environment fingerprinting')
    parser.add_argument('--no-crash', action='store_true', help='Matikan crash injection')
...
╔══════════════════════════════════════════════════════════════╗
║     LUA OBFUSCATOR MAX v4.0 — Ultra Security Edition         ║
║     Input/Output: Penyimpanan Lokal                          ║
╠══════════════════════════════════════════════════════════════╣
║  Mode: {'BATCH (Folder)' if args.directory else 'SINGLE FILE'}                             ║
...
""")
    ...
    if args.max:
        print("  ☢️  MAXIMUM SECURITY MODE AKTIF — Semua layer diaktifkan!")
        opts['str_layers'] = 12
    ...
    if args.directory:
        if not os.path.isdir(args.directory):
            print(f"  ❌ Error: Folder '{args.directory}' tidak ditemukan!")
            sys.exit(1)

        # Find all .lua files
        lua_files = []
        for ext in ['*.lua', '*.luac', '*.txt']:
            lua_files.extend(glob.glob(os.path.join(args.directory, ext)))

        if not lua_files:
            print(f"  ❌ Error: Tidak ada file .lua di folder '{args.directory}'!")
            sys.exit(1)

        print(f"  📁 Ditemukan {len(lua_files)} file .lua\n")
        ...
        for lua_file in sorted(lua_files):
            ...
            output_file = os.path.join(args.output, f"{name}{args.suffix}{ext}")

            if process_file(lua_file, output_file, opts):
                success_count += 1
            else:
                fail_count += 1

        print(f"\n{'='*60}")
        print(f"  BATCH COMPLETE: {success_count} sukses, {fail_count} gagal")
        print(f"  Output folder: {args.output}")
        print(f"{'='*60}\n")

    # SINGLE FILE MODE
    else:
        if process_file(args.input, args.output, opts):
            success_count = 1
        else:
            fail_count = 1
            sys.exit(1)
